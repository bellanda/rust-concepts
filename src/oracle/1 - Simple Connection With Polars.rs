...
fn main() -> Result<(), Box<dyn Error>>
{
    ...
    let username = env::var("ORACLE_USERNAME").expect("Variável de ambiente ORACLE_USERNAME não encontrada");
    let password = env::var("ORACLE_PASSWORD").expect("Variável de ambiente ORACLE_PASSWORD não encontrada");
    ...
    let service_name = env::var("ORACLE_SERVICE_NAME").expect("Variável de ambiente ORACLE_SERVICE_NAME não encontrada");
    ...
    let column_names: Vec<String> = column_info.iter().map(|info| info.name().to_string()).collect();
